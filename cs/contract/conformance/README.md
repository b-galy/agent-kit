...
Verifies that a Castalie MCP endpoint honours the `castalie-pm-v1` contract — and, above all, the outward-only
...
Against a live endpoint — set `CASTALIE_ENDPOINT` (the host) and `CASTALIE_TOKEN`. The runner derives the MCP
url (`<endpoint>/mcp`) and the REST base from it, and exercises both surfaces:

```
CASTALIE_ENDPOINT=https://<your-workspace>.castalie.app CASTALIE_TOKEN=<token> node runner.mjs
```

(You can pin the MCP url separately with `CASTALIE_MCP_URL` if it differs.) Exit code `0` = all checks pass,
