...
- **Read both names, write only the new one.** The markers are `castalie:begin`, `castalie:end`
  and `castalie:instructions`. A block written before 1.17.0 carries the same three under the
  **legacy prefix**, the kit's first name: `g` followed by `aly`, joined into one word. It is given
  in two halves because this repository never spells the retired name whole; in a customer's file
  it appears whole, as `<!-- ` + that word + `:begin -->`, `:end -->` and `:instructions -->`. A
  skill honours those markers exactly as it honours the new ones: customers' files already hold them, and ignoring them
...
1. In the root instruction file — already in your context, so this costs no tool call — find the
   lines carrying the marker, under either name (`<!-- castalie:instructions -->`, or the same
   marker under the legacy prefix above). **Then, if the repository root holds the other spelling as well,
